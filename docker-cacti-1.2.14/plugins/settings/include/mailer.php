...
class Mailer {
	 ...
	 * @return string - body
	 */
	function wordwrap($body) {
		return ($this->Config["Mail"]["WordWrap"] > 0 ? wordwrap($body, $this->Config["Mail"]["WordWrap"]) : $body);
	}
	...
	function send($body, $headers = array()) {
		...
		if (is_array($body)) {
			if (isset($body["text"])) {
				$body_text = $this->wordwrap($body["text"]);
			} else {
				$this->Error = "No text part of body defined";
				return false;
			}
			if (isset($body["html"])) {
				$body_html = $this->wordwrap($body["html"]);
			} else {
				$this->Error = "No html part of body defined";
				return false;
			}
		} else {
			$body_text = $this->wordwrap($body);
		}
		if (empty($body)) {
