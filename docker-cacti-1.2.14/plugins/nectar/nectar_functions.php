...
function nectar_interval_start($interval, $count, $offset, $timestamp) {
	...
	nectar_log(__FUNCTION__ . ", interval: " . $nectar_interval[$interval] . " count: " . $count . " offset: " . $offset . " timestamp: " . date("Y/m/d H:i:s", $timestamp), false, "NECTAR TRACE", POLLER_VERBOSITY_MEDIUM);

	switch ($interval) {
		case NECTAR_SCHED_INTVL_MINUTE:
			# add $count minutes to current mailtime
			$ts = utime_add($timestamp,0,0,0,0,$count,$offset);
			break;
		case NECTAR_SCHED_INTVL_HOUR:
