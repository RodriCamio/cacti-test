...
function form_save_aggregate() {
	...
	$save_to = "plugin_aggregate_graph_templates_item";
	...
	if ($_POST["aggregate_graph_id"] > 0) {
		$save_to = "plugin_aggregate_graphs_graph_item";
	...
	if (!is_error_message()) {
		...
		if ($save_to == "plugin_aggregate_graph_templates_item")
		...
		if ($save_to == 'plugin_aggregate_graphs_graph_item')
			push_out_aggregates(0, $_POST["local_graph_id"]);
		elseif ($save_to == 'plugin_aggregate_graph_templates_item')
			push_out_aggregates($_POST["aggregate_template_id"]);

	}

	if (is_error_message()) {
		header("Location: " . $config['url_path'] . "plugins/aggregate/". $location_failure);
		exit;
	}else{
		header("Location: " . $config['url_path'] . "plugins/aggregate/" . $location_success);
		exit;
	}
}
...
function item_edit() {
	...
	draw_edit_form(
		array(
			"config" => array(
				"post_to" => $config['url_path'] . "plugins/aggregate/aggregate_items.php"
				),
			"fields" => $form_array
			)
		);
	...
	aggregate_save_button($config['url_path'] . "plugins/aggregate/$page_name?action=edit&id=" . get_request_var_request("local_graph_id"));
