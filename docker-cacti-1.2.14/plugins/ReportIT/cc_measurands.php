...
$measurand_actions 	= array( 2 => 'Delete');
...
function standard() {
	...
	$sql = "SELECT * FROM reportit_measurands WHERE template_id={$_GET['id']} ORDER BY id";
	...
	$header_label	= "<b>Measurands </b>[Template: <a style='color:yellow' href='cc_templates.php?action=template_edit&id={$_GET['id']}'>$template_name</a>] [$number_of_measurands]";

	html_start_box("$header_label", REPORTIT_WIDTH, $colors["header"], "2", "center", "cc_measurands.php?action=measurand_edit&template_id={$_GET['id']}");
...
function form_save() {
	...
	input_validate_input_key(get_request_var_post('measurand_type'), $type_specifier);
	input_validate_input_key(get_request_var_post('measurand_precision'), $precision, true);
	input_validate_input_key(get_request_var_post('measurand_rounding'), array(0,1,2), true);

	form_input_validate($_POST['measurand_description'], 'measurand_description', '', false, 3);
	form_input_validate($_POST['measurand_abbreviation'], 'measurand_abbreviation', '^[a-zA-Z0-9]+$', false, 3);
	form_input_validate($_POST['measurand_unit'], 'measurand_unit', '^[\/\\\$a-zA-Z0-9%²³-]+$', false, 3);
	form_input_validate($_POST['measurand_formula'], 'measurand_formula', '', false, 3);
	form_input_validate($_POST['measurand_cf'], 'measurand_cf', '[1-4]', false, 3);
	/* ==================================================== */

	//Check if the abbreviation is in use.
	$sql = "SELECT COUNT(*) FROM reportit_measurands WHERE abbreviation='{$_POST['measurand_abbreviation']}'
			 AND id!='{$_POST['id']}' AND template_id='{$_POST['template_id']}'";
	...
	if(strlen($_POST['measurand_formula'])) {
		$calc 		= $_POST['measurand_formula'];
		$intersizes 	= get_interim_results($_POST['id'], $_POST['template_id']);
		$calc_var_names = array_keys(get_possible_variables($_POST['template_id']));
		$data_query_variables = get_possible_data_query_variables($_POST['template_id']);
	...
	if(!is_error_message_field('measurand_abbreviation') && $_POST['id'] != 0) {
		$dependences 	= array();
		$new		= $_POST['measurand_abbreviation'];

		$sql		= "SELECT abbreviation FROM reportit_measurands WHERE id={$_POST['id']}";
		$old		= db_fetch_cell($sql);

		if($old != $new) {
			$sql	= "SELECT id, calc_formula FROM reportit_measurands WHERE template_id={$_POST['template_id']}
						 AND id >{$_POST['id']}
		...
		if(isset($_POST['measurand_spanned'])) {
			$sql	= "SELECT COUNT(*) FROM reportit_measurands WHERE template_id={$_POST['template_id']}
						 AND id >{$_POST['id']}
						 AND calc_formula LIKE '%$old:%'";
			$count	= db_fetch_cell($sql);
	...
	$measurand_data['id']				= $_POST['id'];
	$measurand_data['template_id']		= $_POST['template_id'];
	$measurand_data['description']		= $_POST['measurand_description'];
	$measurand_data['abbreviation']		= strtoupper($_POST['measurand_abbreviation']);
	$measurand_data['unit']				= $_POST['measurand_unit'];
	$measurand_data['visible']			= isset($_POST['measurand_visible']) ? 1 : 0;
	$measurand_data['spanned']			= isset($_POST['measurand_spanned']) ? 1 : 0;
	$measurand_data['calc_formula']		= $_POST['measurand_formula'];
	$measurand_data['rounding']			= isset($_POST['measurand_rounding']) ? $_POST['measurand_rounding']: '';
	$measurand_data['cf']				= $_POST['measurand_cf'];
	$measurand_data['data_type']		= $_POST['measurand_type'];
	$measurand_data['data_precision']	= isset($_POST['measurand_precision']) ? $_POST['measurand_precision'] : '';

	if(is_error_message()) {
		header("Location: cc_measurands.php?action=measurand_edit&id={$_POST['id']}&template_id={$_POST['template_id']}");
	}else {
		//Save data
		...
		header("Location: cc_measurands.php?&id={$_POST['template_id']}");
		raise_message(1);
	}
}
...
function measurand_edit() {
	...
	if (!empty($_GET['id'])) {
		$measurand_data		= db_fetch_row('SELECT * FROM reportit_measurands WHERE id=' . $_GET['id']);
	...
	$measurand_id		= (isset($_GET['id']) ? $_GET['id'] : '0');
	$template_id		= (isset($_GET['template_id']) ? $_GET['template_id'] : $measurand_data['template_id']);
	...
	html_start_box("<strong>Measurand configuration</strong> $header_label", REPORTIT_WIDTH, $colors["header"], "2", "center", "");
		...
		if(status) {
			for(var i = 0; i < desc.length; i++) {
				if (desc[i]["name"] == name) {
					Tooltip.innerHTML =
						'<br>'
						+'<table width=<?php print REPORTIT_WIDTH; ?> align="center" cellpadding="2" style="background-color:#FFFACD; border: 1px solid #bbbbbb;" >'
...
function form_actions() {
	...
	if (isset($_POST['selected_items'])) {
		...
		if ($_POST['drp_action'] == '2') { // DELETE MEASURANDS
			...
			if(stat_autolock_template($_POST['id'])) set_autolock_template($_POST['id']);
		}
		header("Location: cc_measurands.php?id={$_POST['id']}");
		exit;
	}
	...
	include_once(CACTI_INCLUDE_PATH . '/top_header.php');
	html_start_box("<strong>" . $measurand_actions{$_POST["drp_action"]} . "</strong>", "60%", $colors["header_panel"], "3", "center", "");
