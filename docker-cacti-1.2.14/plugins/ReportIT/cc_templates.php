...
function template_export() {
	...
	$info = array();
	$info['description']	= $_POST['template_description'];
	$info['author']			= $_POST['template_author'];
	$info['version']		= $_POST['template_version'];
	$info['contact']		= $_POST['template_contact'];

	$output = export_report_template($_POST['template_id'], $info);
...
function template_import() {
	...
	/* ================= input validation ================= */
	input_validate_input_number(get_request_var_post('data_template'));
	/* ==================================================== */
	...
	$template_data['id'] = 0;
	$template_data['data_template_id'] = $_POST['data_template'];
	...
	if(is_array($template_data_source_items)) {
		if(!isset($template_data_source_items['data_source_item'][0])) {
			...
			$sql = "SELECT id FROM `data_template_rrd`
					...
					AND data_template_id = {$_POST['data_template']}
...
function standard() {
	...
	input_validate_input_whitelist(get_request_var_request("sort"), $link_array, true);
	input_validate_input_whitelist(get_request_var_request("mode"), $order_array, true);
	/* ==================================================== */
	...
	/* generate page list */
	$url_page_select = html_custom_page_list($_REQUEST["page"], MAX_DISPLAY_PAGES, $session_max_rows, $total_rows, "cc_templates.php?");
	...
	$nav = "<tr bgcolor='#6CA6CD' >
		<td colspan='8'>
			<table width='100%' cellspacing='0' cellpadding='0' border='0'>
				<tr>
					<td align='left'>
						<strong>&laquo; "; if ($_REQUEST["page"] > 1) { $nav .= "<a style='color:FFFF00' href='cc_templates.php?page=" . ($_REQUEST["page"]-1) . "'>"; } $nav .= "Previous"; if ($_REQUEST["page"] > 1) { $nav .= "</a>"; } $nav .= "</strong>
					</td>\n
					...
					<td align='right'>
						<strong>"; if (($_REQUEST["page"] * $session_max_rows) < $total_rows) { $nav .= "<a style='color:yellow' href='cc_templates.php?page=" . ($_REQUEST["page"]+1) . "'>"; } $nav .= "Next"; if (($_REQUEST["page"] * $session_max_rows) < $total_rows) { $nav .= "</a>"; } $nav .= " &raquo;</strong>
					</td>\n
				</tr>
			</table>
		</td>
	...
	html_custom_header_box("<strong>Report Templates </strong>[$total_rows]", false, $hyperlinks, $images);
	print $nav;
...
function form_save() {
	...
	input_validate_input_number(get_request_var_post('template_data_template'));
	form_input_validate($_POST['template_description'], 'template_description', '', false, 3);
	form_input_validate($_POST['template_filter'], 'template_filter', '', true, 3);
	/* ==================================================== */

	$template_data						= array();
	$template_data['id']				= $_POST['id'];
	$template_data['description']		= $_POST['template_description'];
	$template_data['pre_filter']		= $_POST['template_filter'];
	$template_data['data_template_id']	= $_POST['data_template_id'];
	$template_data['locked']			= isset($_POST['template_locked']) ? 1 : 0;
	$template_data['export_folder']		= isset($_POST['template_export_folder']) ? mysql_real_escape_string($_POST['template_export_folder']) : '';
	...
	foreach($_POST as $key => $value){
		if(strpos($key, 'ds_enabled__') !== false) {
			...
			$ds_items[$ds_id]['data_source_alias']	= mysql_real_escape_string(trim($_POST[$ds_alias]));
		}
	}
	...
	if(is_error_message()) {
		header('Location: cc_templates.php?action=template_edit&id=' . $_POST['id']);
	}else {
		/* save template data */
...
function template_edit() {
	...
	if (!empty($_GET['id'])) {
		$template_data = db_fetch_row('SELECT * FROM reportit_templates WHERE id=' . $_GET['id']);
	...
	if(isset($_POST['data_template'])) {
		if(!isset($_SESSION['reportit_tWizard']['data_template']))
		$_SESSION['reportit_tWizard']['data_template'] = $_POST['data_template'];
	}
	...
	$form_array = array(
		...
		'method'				=> 'checkbox',
		'default'				=> 'on',
		'description'			=> 'Define this template as locked (default), so that power users <br>
									can\'t use it until you have checked its functionality. During that time they are<br>
									also not allowed to modify or run reports based on this template.',
		...
		'method'				=> 'textbox',
		'max_length'			=> '100',
		'description'			=> 'Optional: The syntax to filter the available list of data items<br>
									by their description. Use SQL wildcards like % and/or _. No regular Expressions!',
	...
	if(read_config_option('reportit_auto_export')) {
		...
		"description"			=> "Optional: The path to an folder for saving the exports.
									If it does not exist ReportIt automatically tries to create it during the first scheduled calculation, else
									it will try to create a new subfolder within the main export folder using the template id.",
		"method"				=> "dirpath",
		"max_length"			=> "255",
	...
	if(!empty($_GET['id'])) {
		//Build "Create links"
		$links		= array();
		$href		= 'cc_variables.php?action=variable_edit&template_id=' . $_GET['id'];
		$text		= 'Create a new variable';
		$links[]	= array('href' =>$href, 'text' =>$text);

		$href		= 'cc_measurands.php?action=measurand_edit&template_id=' . $_GET['id'];
	...
	html_start_box("<b>Template configuration</strong> $header_label", REPORTIT_WIDTH, $colors["header"], "2", "center", "");
...
function form_actions() {
	...
	if (isset($_POST['selected_items'])) {
		...
		if ($_POST['drp_action'] == '1') { // DELETE REPORT TEMPLATE
		...
		}elseif ($_POST['drp_action'] == '2') { //DUPLICATE REPORT TEMPLATE
			...
			for ($i=0;($i<count($selected_items));$i++) {
				...
				$template_data['id'] = 0;
				$template_data['description'] = str_replace("<template_title>", $template_data['description'], $_POST['template_addition']);
				$template_id = sql_save($template_data, 'reportit_templates');
				...
				$template_ds_items = db_fetch_assoc('SELECT * FROM reportit_data_source_items WHERE template_id = ' . $selected_items[$i] . ' ORDER BY id');
	...
	html_start_box("<strong>" . $template_actions{$_POST["drp_action"]} . "</strong>", "60%", $colors["header_panel"], "3", "center", "");
