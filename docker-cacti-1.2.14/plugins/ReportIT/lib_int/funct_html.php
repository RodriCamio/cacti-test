...
function html_custom_header_box($header, $hlink = false, $href = '', $link = '') {
    ...
    echo "<table align='center' width=" . REPORTIT_WIDTH . " cellpadding='1' cellspacing='0' border='0' bgcolor='#{$colors['header']}'>
            <tr>
                <td>
...
function html_report_start_box() {
    global $colors;
    echo "<table align='center' width=" . REPORTIT_WIDTH . " cellpadding='3' cellspacing='1' bgcolor='#{$colors['header_panel']}'>";
}


function html_blue_link(& $data, $lb=true) {
    // Create a blue HTML link in an existing table
    echo   "<table width=" . REPORTIT_WIDTH . " align='center'>
	    		<tr>
...
function html_sorted_with_arrows(& $desc_array, & $link_array, $page, $id = FALSE) {
	...
	$asc	= (isset($_REQUEST['sort']) && isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'ASC') ? $_REQUEST['sort'] : '';
	$desc	= (isset($_REQUEST['sort']) && isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'DESC') ? $_REQUEST['sort'] : '';
...
function html_custom_form_button($cancel_url="", $force_type = "", $key_field = "id", $leading_br = false, $width=REPORTIT_WIDTH) {
	global $config;
