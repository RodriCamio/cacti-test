...
/*
   ...
   +-------------------------------------------------------------------------+
*/
define("EVENT_SEVERITY_LOW", 1);
define("EVENT_SEVERITY_MEDIUM", 2);
define("EVENT_SEVERITY_HIGH", 3);
define("EVENT_SEVERITY_CRITICAL", 4);

function plugin_snmpagent_install() {

	/* updates based on user actions */
	api_plugin_register_hook('snmpagent', 'config_settings', 			'snmpagent_config_settings', 			'setup.php');
	api_plugin_register_hook('snmpagent', 'config_insert',				'snmpagent_config_insert', 				'setup.php');
	api_plugin_register_hook('snmpagent', 'config_arrays',				'snmpagent_config_arrays', 				'setup.php');
	api_plugin_register_hook('snmpagent', 'draw_navigation_text',		'snmpagent_draw_navigation_text', 		'setup.php');
	api_plugin_register_hook('snmpagent', 'global_settings_update', 	'snmpagent_global_settings_update', 	'setup.php');
	api_plugin_register_hook('snmpagent', 'device_action_bottom', 		'snmpagent_device_action_bottom', 		'setup.php');
	api_plugin_register_hook('snmpagent', 'api_device_new', 			'snmpagent_api_device_new', 			'setup.php');
	api_plugin_register_hook('snmpagent', 'data_source_action_bottom', 	'snmpagent_data_source_action_bottom', 	'setup.php');
	api_plugin_register_hook('snmpagent', 'graphs_action_bottom', 		'snmpagent_graphs_action_bottom', 		'setup.php');
	/* poller related updates */
	api_plugin_register_hook('snmpagent', 'poller_exiting', 			'snmpagent_poller_exiting', 			'setup.php');
	api_plugin_register_hook('snmpagent', 'poller_bottom', 				'snmpagent_poller_bottom', 				'setup.php');
	api_plugin_register_hook('snmpagent', 'cacti_stats_update', 		'snmpagent_cacti_stats_update', 		'setup.php');
	api_plugin_register_hook('snmpagent', 'utilities_list', 			'snmpagent_utilities_list', 			"setup.php");
	api_plugin_register_hook('snmpagent', 'utilities_action', 			'snmpagent_utilities_action', 			"setup.php");

	api_plugin_register_realm('snmpagent', 'managers.php', 'Plugin -> SNMPAgent: Configure SNMP Notification Receivers', 1);

	snmpagent_system_setup();
	snmpagent_cache_install();
}

function plugin_snmpagent_uninstall () {
	/* nothing to do here - everything will be handled by PIA */
	return true;
}

function plugin_snmpagent_check_config () {
	/* Here we will check to ensure everything is configured */
	snmpagent_check_upgrade();
	return true;
}

function plugin_snmpagent_upgrade () {
	/* Here we will upgrade to the newest version */
	snmpagent_check_upgrade();
	return false;
}

function plugin_snmpagent_version () {
	return snmpagent_version();
}

function snmpagent_check_upgrade () {
	global $config;
	...
	$current = plugin_snmpagent_version();
	$current = $current['version'];
	$old = db_fetch_row("SELECT * FROM plugin_config WHERE directory='snmpagent'");

	if (sizeof($old) && $current != $old["version"]) {
		/* if the plugin is installed and/or active */
		if ($old["status"] == 1 || $old["status"] == 4) {
			/* re-register the hooks */
			plugin_snmpagent_install();
			/* perform a database upgrade */
			require_once($config['base_path'] . '/plugins/snmpagent/system/upgrade.php');
			snmpagent_system_upgrade($old);
		}
		/* update the plugin information */
		$info = plugin_snmpagent_version();
		$id = db_fetch_cell("SELECT id FROM plugin_config WHERE directory='snmpagent'");
...
function snmpagent_check_dependencies() {
	global $plugins, $config;
	return true;
}

function snmpagent_system_setup() {
	global $config;
	require_once($config['base_path'] . '/plugins/snmpagent/system/install.php');
	snmpagent_system_install();
}

function snmpagent_version () {
	return array(
		'name'     => 'snmpagent',
		'version'  => '0.2.2',
		'longname' => 'Cacti SNMP Agent',
...
function snmpagent_config_settings(){
	global $tabs, $settings, $config, $snmp_auth_protocols, $snmp_priv_protocols;

	$temp = array(
		"snmpagent_general_header" => array(
			"friendly_name" => 'SNMPAgent',
			"method" => "spacer",
			),
		'snmpagent_path_snmptrap' => array(
			'friendly_name' => 'SNMP Trap Binary Path',
...
function snmpagent_config_insert(){
	global $config, $menu;
	include_once($config["base_path"] . "/plugins/snmpagent/include/constants.php");
	include_once($config["base_path"] . "/plugins/snmpagent/lib/mib_cache.php");
}

function snmpagent_utilities_list() {
	...
	<tr bgcolor="#<?php print $colors["form_alternate2"];?>">
		<td class="textArea">
			<a href='utilities.php?action=view_snmpagent_cache'>View SNMPAgent Cache</a>
		</td>
		<td class="textArea">
	...
	<tr bgcolor="#<?php print $colors["form_alternate1"];?>">
		<td class="textArea">
			<a href='utilities.php?action=view_snmpagent_events'>View SNMPAgent Notification Log</a>
		</td>
		<td class="textArea">
	...
	<tr bgcolor="#<?php print $colors["form_alternate2"];?>">
		<td class="textArea">
			<a href='utilities.php?action=rebuild_snmpagent_cache'>Rebuild SNMPAgent Cache</a>
		</td>
		<td class="textArea">
...
function snmpagent_utilities_action($action) {
	global $config, $colors;

	$options = array(
		'view_snmpagent_status',
		'view_snmpagent_cache',
		'rebuild_snmpagent_cache',
		'view_snmpagent_events');
	...
	if ( in_array($action,$options) ) {
		...
		load_current_session_value("refresh", "sess_snmpagent_utilities_refresh", "60");

		$refresh["seconds"] = $_REQUEST["refresh"];
		$refresh["page"] = "utilities.php?action=view_snmpagent_status";
		include_once($config['base_path'] . "/plugins/snmpagent/lib/utilities.php");
...
function snmpagent_draw_navigation_text ($nav) {
	$nav["utilities.php:view_snmpagent_cache"] = array("title" => "SNMPAgent Cache", "mapping" => "index.php:,utilities.php:", "url" => "utilities.php", "level" => "1");
	$nav["utilities.php:rebuild_snmpagent_cache"] = array("title" => "SNMPAgent Cache", "mapping" => "index.php:,utilities.php:", "url" => "utilities.php", "level" => "1");
	$nav["utilities.php:view_snmpagent_events"] = array("title" => "SNMPAgent Notification Log", "mapping" => "index.php:,utilities.php:", "url" => "utilities.php", "level" => "1");

	$nav["managers.php:"] = array("title" => "SNMP Notification Receivers", "mapping" => "index.php:", "url" => "utilities.php", "level" => "1");
	$nav["managers.php:edit"] = array("title" => "SNMP Notification Receivers (edit)", "mapping" => "index.php:", "url" => "utilities.php", "level" => "1");
	$nav["managers.php:save"] = array("title" => "SNMP Notification Receivers", "mapping" => "index.php:", "url" => "utilities.php", "level" => "1");
	$nav["managers.php:actions"] = array("title" => "SNMP Notification Receivers", "mapping" => "index.php:", "url" => "utilities.php", "level" => "1");
	return $nav;
}

function snmpagent_config_arrays () {
	global $menu;
	/* show additional console entries only if this plugin has been enabled */
	if(api_plugin_is_enabled('snmpagent')) {
		$menu['Configuration']['plugins/snmpagent/lib/managers.php'] = 'SNMP Notification Receivers';
	}
}
...
function snmpagent_cache_install(){
	...
	/* drop everything */
	db_execute("TRUNCATE `plugin_snmpagent_cache`");
	db_execute("TRUNCATE `plugin_snmpagent_mibs`;");
	db_execute("TRUNCATE `plugin_snmpagent_cache_notifications`;");
	db_execute("TRUNCATE `plugin_snmpagent_cache_textual_conventions`;");

	$mc = new MibCache();
	$mc->install($config["base_path"] . '/plugins/snmpagent/mibs/CACTI-MIB');
	$mc->install($config["base_path"] . '/plugins/snmpagent/mibs/CACTI-SNMPAGENT-MIB');
...
function snmpagent_notification( $notification, $mib, $varbinds, $severity = EVENT_SEVERITY_MEDIUM){
	global $config;
	...
	if( !in_array( $severity, array( EVENT_SEVERITY_LOW, EVENT_SEVERITY_MEDIUM, EVENT_SEVERITY_HIGH, EVENT_SEVERITY_CRITICAL ) ) ) {
	...
	$enterprise_oid = db_fetch_cell("SELECT oid from plugin_snmpagent_cache where `name` = '" . $notification . "' AND `mib` = '" . $mib ."'");
	if(!$enterprise_oid) {
	...
	$sql = "SELECT plugin_snmpagent_managers.*
				FROM plugin_snmpagent_managers_notifications
					INNER JOIN plugin_snmpagent_managers
					ON (
						plugin_snmpagent_managers.id = plugin_snmpagent_managers_notifications.manager_id
					)
				WHERE plugin_snmpagent_managers.disabled = 0 AND plugin_snmpagent_managers_notifications.notification = '$notification' AND plugin_snmpagent_managers_notifications.mib = '$mib'";

	$notification_managers = db_fetch_assoc($sql);

	if(!$notification_managers) {
		/* To bad! Nobody wants to hear our message. :( */
		if(in_array($severity, array(EVENT_SEVERITY_HIGH, EVENT_SEVERITY_CRITICAL))) {
			if(read_config_option('log_verbosity')>POLLER_VERBOSITY_NONE) {
	...
	$sql = "SELECT
					plugin_snmpagent_cache_notifications.attribute,
					plugin_snmpagent_cache.oid,
					plugin_snmpagent_cache.type,
					plugin_snmpagent_cache_textual_conventions.type as tcType
			FROM plugin_snmpagent_cache_notifications
				LEFT JOIN plugin_snmpagent_cache
				ON (
					plugin_snmpagent_cache.mib = plugin_snmpagent_cache_notifications.mib
					AND
					plugin_snmpagent_cache.name = plugin_snmpagent_cache_notifications.attribute
				)
				LEFT JOIN plugin_snmpagent_cache_textual_conventions
				ON (
					plugin_snmpagent_cache.mib = plugin_snmpagent_cache_textual_conventions.mib
					AND
					plugin_snmpagent_cache.type = plugin_snmpagent_cache_textual_conventions.name
				)
			WHERE plugin_snmpagent_cache_notifications.name = '$notification' AND plugin_snmpagent_cache_notifications.mib = '$mib'
			ORDER BY plugin_snmpagent_cache_notifications.sequence_id";
	...
	if(sizeof($difference) == 0) {
		...
		*/
		$sql = "SELECT plugin_snmpagent_managers.* FROM plugin_snmpagent_managers_notifications
					INNER JOIN plugin_snmpagent_managers
					ON (
						plugin_snmpagent_managers.id = plugin_snmpagent_managers_notifications.manager_id
					)
				WHERE plugin_snmpagent_managers_notifications.notification = '$notification' AND plugin_snmpagent_managers_notifications.mib = '$mib'
				ORDER BY plugin_snmpagent_managers.snmp_message_type";
		...
		if($notification_managers && sizeof($notification_managers)>0) {
			...
			*/
			...
			foreach($notification_managers as $notification_manager) {
				...
				sql_save( $save, 'plugin_snmpagent_notifications_log');
